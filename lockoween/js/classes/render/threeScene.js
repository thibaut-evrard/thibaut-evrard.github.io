...
export default class ThreeScene {
  ...
  constructor( width, height, isMobile ) {
    ...
    this.setupLights();
    this.face = new Face( this.scene, isMobile );
    this.text = new House( this.scene, isMobile );
  ...
  update( metrics ) {
    ...
    const offset = metrics.points.offset;
    const rotation = metrics.orientation.rotation;

    if( this.isMobile == false) {
      this.cameraTarget.set( -offset.x / 2, -offset.y / 2, this.cameraTarget.z );
    }
    else {
      this.camera.position.x = rotation.y / 2;
      this.camera.position.y = -rotation.x / 2;
